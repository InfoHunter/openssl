...
Legacy setter methods SSL_CTX_set_cookie_generate_cb() and SSL_CTX_set_cookie_verify_cb()
are still available for current applications who want support both TLS1.2 with stateful
cookies and TLS1.3 with stateless cookies.

=head1 SEE ALSO

L<SSL_stateless(3)> L<SSL_CTX_set_cookie_generate_cb(3)>
